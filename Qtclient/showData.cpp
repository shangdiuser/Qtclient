...
void showData::initiShow()
{
   ...
   if (parseError.error == QJsonParseError::NoError) {
       if (jsonDoc.isArray()) {
           ...
           // 设置表头
           model->setHorizontalHeaderLabels({ "员工编号", "姓名", "部门", "上班时间", "是否迟到" });
           ...
           for (const auto& value : jsonArray) {
               if (value.isObject()) {
                   ...
                   model->setItem(row, 2, new QStandardItem(department));
                   model->setItem(row, 3, new QStandardItem(punchInTime));
                   model->setItem(row, 4, new QStandardItem(isLate));
